  ...
  return aboutMe.map((Item, index) => (
    <div key={Item.id}>
      <motion.div
        ...
        className={`${
          styles.flexCenter
        } flex-col p-[10px] py-[20px] rounded-[15px] w-[230px] sm:w-[200px] ${
          index !== aboutMe.length - 1 ? "sm:mr-[20px]" : "sm:mr-0"
        } mt-[20px] md:mt-0 skill-card`}
      >
        <Item.iconType className="fill-white w-[35px] h-[35px] mb-[10px]" />
        <h4
          className={`${styles.heading5} leading-[15px] text-[18px] sm:text-[20px]`}
        >
          {Item.title}
        </h4>
        <p
          className={`${styles.paragraph} text-[14.8px] sm:text-[14px] text-background font-normal`}
        >
          {Item.description}
        </p>
      </motion.div>
